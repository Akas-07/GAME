class GameBoard {
	...
	setPodium = (newPlayer) => {
		if (!this.podium.includes(newPlayer)) {
			...
			document
				.querySelector('#gamePodium')
				.appendChild(currentFinisher.getPiece());
		}
	...
	gameOver = async () => {
		...
		for (let playerName in this.playerPositions) {
			if (this.playerPositions[playerName] === 100) {
				...
				document
					.querySelector('#gamePodium')
					.removeChild(currentFinisher.getPiece());
				document
	...
	updateTurn = async () => {
		if (
			this.podium.includes(this.playerNames[this.currentPlayerTurn]) === false
		) {
			for (let playerName in this.players) {
				let player = this.players[playerName];
			...
			if (this.numberOfPlayers === 1 && this.currentPlayerTurn === 1) {
				...
				this.players[this.playerNames[this.currentPlayerTurn]]
					.getPiece()
	...
	playGame = async (player) => {
		...
		document.getElementById('dice').style.backgroundPositionX = `${
			this.diceImagePositions[diceRoll - 1]
		}px`;
		...
		if (finalPosition <= 100) {
			if (player.getPosition() === 0) {
				...
				for (
					let i = this.playerPositions[player.getName()];
					i <= finalPosition;
					i++
				) {
		...
		if (this.playerPositions[player.getName()] < 100) {
			...
			if (
				this.playerPositions[player.getName()] in
				this.board.getSnakeAndLadders()
			) {
				this.playerPositions[player.getName()] =
					this.board.getSnakeAndLadders()[
						this.playerPositions[player.getName()]
					];
				player.setPosition(this.playerPositions[player.getName()]);
			...
			let msg = `[${new Date().toLocaleTimeString()}] Player rolled a ${diceRoll}. Current Position: ${
				this.playerPositions[player.getName()]
			} <br/>`;
		...
		if ((diceRoll !== 6 && !isCaptured) || player.getPosition() >= 100) {
			...
			do {
				...
				let calculatedPlayer =
					this.numberOfPlayers === 1 ? 2 : this.numberOfPlayers;
				if (
					this.podium.length === calculatedPlayer ||
					this.isGameOver === true
				) {
					this.gameOver();
					return;
				...
				playerName =
					this.playerNames[
						this.numberOfPlayers === 1 && this.currentPlayerTurn === 1
							? 4
							: this.currentPlayerTurn
					];
	...
	playerRoll = () => {
		...
		const redPos = this.playerPositions['red'];
		const greenPos = this.playerPositions['green'];
		const bluePos = this.playerPositions['blue'];
		const yellowPos = this.playerPositions['yellow'];
		const rulesList = document.getElementById('rulesList1');
		...
		const rules2 = [...rulesList2.children];
		console.log(rules);
		rulesView.innerHTML = '';
		rulesView.style.display = 'none';
		rules.forEach((rule) => {
			const ruleNumber = Number(rule.textContent.split('.')[0]);

			if (ruleNumber === redPos) {
				console.log(ruleNumber, redPos);
				console.log(rule);
				rule.classList.add('highlight');

				setTimeout(() => {
					rulesView.style.display = 'block';
					rulesView.innerHTML = rule.textContent;
				}, 0);
				setTimeout(() => {
					rulesView.style.display = 'none';
				}, 6000);
				...
				console.log(ruleNumber, redPos);
				rule.classList.remove('highlight');
			}
		});
		rules2.forEach((rule) => {
			const ruleNumber = Number(rule.textContent.split('.')[0]);
			if (ruleNumber === redPos) {
				...
				setTimeout(() => {
					rulesView.style.display = 'block';
					rulesView.append(rule.textContent);
				}, 0);
				setTimeout(() => {
					rulesView.style.display = 'none';
				}, 6000);
	...
	initializeGame = () => {
		...
		const redPlayerPiece =
			document.getElementById('redPlayerPiece'); /* Red Piece */
		const greenPlayerPiece =
			document.getElementById('greenPlayerPiece'); /* Green Piece */
		const bluePlayerPiece =
			document.getElementById('bluePlayerPiece'); /* Blue Piece */
		const yellowPlayerPiece =
			document.getElementById('yellowPlayerPiece'); /* Yellow Piece */
		const computerPlayerPiece = document.getElementById(
			'computerPlayerPiece',
		); /* Computer Piece */

		const redPlayerBtn = document.getElementById('red'); /* Red Play Button */
		const greenPlayerBtn =
			document.getElementById('green'); /* Green Play Button */
		const playerBlueBtn =
			document.getElementById('blue'); /* Blue Play Button */
		const playerYellowBtn =
			document.getElementById('yellow'); /* Yellow Play Button */
		const computerPlayerBtn =
			document.getElementById('computer'); /* Computer Play Button */
