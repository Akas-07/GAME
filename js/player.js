class Player {
	...
	//const redPlayerPosition = gameBoard.playerPositions['red'];
	//console.log(redPlayerPosition);
	getIndex() {
		return this.index;
	}
	...
	updatePosition() {
		...
		if (this.position === 0) {
			...
			if (this.position % 10 !== 0) {
				this.piece.style.bottom =
					parseInt(this.position / 10) * scaleSize + 'px';
			} else {
				this.piece.style.bottom =
					(parseInt(this.position / 10) - 1) * scaleSize + 'px';
			}

			if (
				(this.position >= 1 && this.position < 10) ||
				(this.position >= 21 && this.position < 30) ||
				(this.position >= 41 && this.position < 50) ||
				(this.position >= 61 && this.position < 70) ||
				(this.position >= 81 && this.position < 90)
			) {
				this.piece.style.left =
					parseInt(this.position % 10) * scaleSize - scaleSize + 'px';
			} else {
				if (parseInt(this.position % 20) === 0) {
					this.piece.style.left = '0px';
				} else if (
					parseInt(this.position % 20) !== 0 &&
					parseInt(this.position % 10) === 0
				) {
					this.piece.style.left = scaleSize * 9 + 'px';
				} else {
					this.piece.style.left =
						scaleSize * 10 - parseInt(this.position % 10) * scaleSize + 'px';
				}
			}
		}
	}
